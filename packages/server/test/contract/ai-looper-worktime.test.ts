...
describe("AI Looper worktime contract", () => {
  test("documents POST /api/ai-looper/runs/{taskRunID}/worktime", () => {
    const operation = (OpenApi.fromApi(Api) as OpenApiSpec).paths["/api/ai-looper/runs/{taskRunID}/worktime"]?.post
    ...
    expect(operation?.operationId).toBe("v2.aiLooper.run.worktime")
    expect(operation?.parameters).toContainEqual(
      expect.objectContaining({ name: "taskRunID", in: "path", required: true }),
    )
    expect(operation?.requestBody?.required).toBe(true)
    expect(payload?.required).toEqual(["worktime_draft_id", "confirmed_minutes", "confirmed_description"])
